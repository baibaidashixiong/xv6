...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    pa = PTE2PA(*pte);//虚拟地址转换为物理地址
    *pte=(*pte & ~PTE_W ) | PTE_COW;
    //清除父进程的PTE_W位，设置PTE_COW位
    flags = PTE_FLAGS(*pte);//做掩码计算（即去除比0x3ff高的位）
    //完成物理地址到虚拟地址到映射
...
uvmcheckcowpage(uint64 va){
  ...
  return va < p->sz //在进程内存堆范围内
    && ((pte=walk(p->pagetable,va,0))!=0) //无page fault
    && (*pte & PTE_V) //页表项合法
    && (*pte & PTE_COW); //页表项为lazy copy
}
